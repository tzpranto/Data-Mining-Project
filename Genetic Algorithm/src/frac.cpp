 ...
 *  we update the IF s.t. the keywords in o are removed (note that the root must be removed), then o is added 
 to S_0, then we recursive call the function to find another object in the (new) root and update IF and add 
 to S_0...., until the IF is empty, then S covers all the keywords.
 * note that if we do not consider the distance constraint, we can always find a feaisble set 
 (e.g., by adding all objects in IF into S). When we consider the distance constraint, we may return NULL.
 */
obj_set_t *combinatorial_sub(bst_t *IF_v, obj_set_t *S_0, obj_t *o, B_KEY_TYPE d)
{
    ...
    bst_node_v = IF_v->root;
    obj_node_v = bst_node_v->p_list_obj->head->next;
    if (bst_node_v->p_list_obj->obj_n > 4)
        printf("%d\n", bst_node_v->p_list_obj->obj_n);
    int cnt = 0;
    while (obj_node_v != NULL)
    {
        // cnt++;
        ...
        if (S != NULL)
        {
            ...
            if (counter == 500)
            {
                ...
                // while( obj_node_v != NULL)
                // {
                //     printf("%d %u ",obj_node_v->obj_v->id, obj_node_v->obj_v->fea);
                //     bst_node_v = bst_search(IF_v, obj_node_v->obj_v->fea);
                //     printf("%d\n", bst_node_v->p_list_obj->obj_n);
                //     obj_node_v = obj_node_v->next;
                // }
...
bool genetic(fsi_t *fsi_v, obj_t *obj_v, obj_set_t *S2, bool *RI)
{
    bst_t *inverted_list;
    obj_set_t *S_0, *S = NULL;
    obj_set_t *obj_set_v;
    ...
    // Invoke the sub-procedure "recursively".
    S = genetic_sub(fsi_v, inverted_list, S_0, obj_v, dist_thr);
...
obj_set_t **initialize(fsi_t *fsi_v, bst_t *IF_v, obj_t *o, int size)
{
    int i, j, k;
    obj_set_t **P = new obj_set_t *[size];

    for (i = 0; i < size; i++)
    {
        obj_set_t *S = alloc_obj_set();

        for (j = 0; j < fsi_v->fea_n; j++)
        {
            obj_t *obj_v;
            bst_node_t *bst_node_v;
            obj_node_t *obj_node_v;

            if (fsi_v->feaset[j] == o->fea)
                continue;

            bst_node_v = bst_search(IF_v, fsi_v->feaset[j]);

            // if (bst_node_v == NULL || bst_node_v->p_list_obj->obj_n == 0) {
            //     printf("Something Went Wrong");
            // }
            obj_node_v = bst_node_v->p_list_obj->head->next;
            int ri = rand() % (int)bst_node_v->p_list_obj->obj_n;
            ...
            //printf("%u %d, ",fsi_v->feaset[j], ri);
            //printf("%u, ",fsi_v->feaset[j]);
        }
...
double fitness(obj_set_t *P)
{
    double score = 0;

    obj_node_t *obj_node_v1 = P->head->next;
    obj_node_t *obj_node_v2;
    ...
    while (obj_node_v1 != NULL)
    {
        ...
        while (obj_node_v2 != NULL)
        {
            score += calc_dist_obj(obj_node_v1->obj_v, obj_node_v2->obj_v);
            obj_node_v2 = obj_node_v2->next;
        }
...
obj_set_t *tournament(obj_set_t **P, int t_size, int popsize)
{
    obj_set_t *best = NULL;
...
obj_set_t *mutate(bst_t *IF_v, obj_set_t *S, obj_t *o)
{
    int i, j, k;

    int index = rand() % S->obj_n;
    obj_node_t *to_replace = S->head->next;
    obj_set_t *N = alloc_obj_set();
    ...
    while (to_replace != NULL)
    {
        if (false) //i == index)
        {
            //printf("Okay So far\n");
            bst_node_t *bst_node_v;
            obj_node_t *with_replace;
            obj_t *obj_v = to_replace->obj_v;

            if (obj_v->fea == o->fea)
            {
                add_obj_set_entry(o, N);
            }
            ...
            else
            {
                ...
                if (bst_node_v == NULL)
                    printf("Error at BST\n");
                with_replace = bst_node_v->p_list_obj->head->next;

                k = rand() % bst_node_v->p_list_obj->obj_n;
                j = 0;
                //printf("%d\n",k);
                while (with_replace != NULL)
                {
                    if (j == k)
                    {
                        add_obj_set_entry(with_replace->obj_v, N);
                        break;
                    }
                    j++;
                    with_replace = with_replace->next;
                }
            }
        }
        else
        {
            add_obj_set_entry(to_replace->obj_v, N);
        }
...
obj_set_t **crossover(obj_set_t *P1, obj_set_t *P2)
{
    obj_set_t **children = new obj_set_t *[2];
    int i, j, k;
    int lower = rand() % P1->obj_n;
    int upper = rand() % P1->obj_n;

    // int count = 0;
    // obj_node_t *p1 = P1->head->next;
    ...
    if (P1->obj_n != P2->obj_n)
    {
        printf("Something is wrong inside Crossover!\n");
        children[0] = P1;
        children[1] = P2;
        return children;
    }
    ...
    obj_node_t *p1 = P1->head->next;
    obj_node_t *p2 = P2->head->next;

    obj_set_t *C1 = alloc_obj_set();
    obj_set_t *C2 = alloc_obj_set();

    for (i = 0; i < lower; i++)
    {
        if (p1->obj_v == NULL || p2->obj_v == NULL)
        {
            printf("Error in Loop1\n");
        }
        add_obj_set_entry(p1->obj_v, C1);
        add_obj_set_entry(p2->obj_v, C2);
        p1 = p1->next;
        p2 = p2->next;
    }
    ...
    for (i = lower; i < upper; i++)
    {
        if (p1 == NULL || p2 == NULL)
        {
            printf("Error in Loop2\n");
        }
        add_obj_set_entry(p1->obj_v, C2);
        add_obj_set_entry(p2->obj_v, C1);
        p1 = p1->next;
        p2 = p2->next;
    }

    for (i = upper; i < P1->obj_n; i++)
    {
        if (p1 == NULL || p2 == NULL)
        {
            printf("Error in Loop1\n");
...
bool check_validity(obj_set_t *P, obj_t *o, B_KEY_TYPE d)
{
    obj_node_t *obj_node_v = P->head->next;

    while (obj_node_v != NULL)
    {
        obj_t *obj_v = obj_node_v->obj_v;

        if (!check_dist_constraint(P, obj_v, o, d))
        {
            return false;
        }
...
obj_set_t *genetic_sub(fsi_t *fsi_v, bst_t *IF_v, obj_set_t *S_0, obj_t *o, B_KEY_TYPE d)
{
    int popsize = 6;
    int iter = 5;
    int t_size = 2;
    int i, j, k;
    obj_set_t **P = initialize(fsi_v, IF_v, o, popsize);
    obj_set_t *best = NULL;
    ...
    for (i = 0; i < iter; i++)
    {
        for (j = 0; j < popsize; j++)
        {
            double ft_score = fitness(P[j]);

            if (check_validity(P[j], o, d))
            {
                return P[j];
            }
            //printf("%lf\n",ft_score);

            if (best == NULL || ft_score > best_fitness)
            {
                best_fitness = ft_score;
                best = P[j];
            }
        }
        ...
        obj_set_t **Q = new obj_set_t *[popsize];

        for (j = 0; j < popsize; j += 2)
        {
            obj_set_t *o1 = tournament(P, t_size, popsize);
            obj_set_t *o2 = tournament(P, t_size, popsize);

            if (o1 == NULL or o2 == NULL)
                printf("Null value in tournament\n");

            obj_set_t **children = crossover(o1, o2);
    ...
    // bst_node_v = IF_v->root;
    // obj_node_v = bst_node_v->p_list_obj->head->next;
    // //if(bst_node_v->p_list_obj->obj_n > 4) printf("%d\n",bst_node_v->p_list_obj->obj_n);
    // int cnt = 0;
    // while (obj_node_v != NULL) {
    //     // cnt++;
    //     // if(cnt>4){
...
bool filter_and_verify(int alg_opt2, fsi_t *fsi_v, obj_t *obj_v, bool *RI)
{
    ...
    if (alg_opt2 == 1)
        ...
        feasibleflag3 = genetic(fsi_v, obj_v, obj_set_v, RI);
